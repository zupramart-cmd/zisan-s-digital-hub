...
// Quick keywords for users to click
const quickKeywords = {
  en: [
  ...
  bn: [
    { label: 'প্রোফাইল', key: 'profile' },
    { label: 'শিক্ষা', key: 'education' },
    { label: 'দক্ষতা', key: 'skills' },
    { label: 'প্রজেক্ট', key: 'projects' },
...
const Chatbot: React.FC = () => {
  ...
  const welcomeMessage = language === 'en' 
    ? "Hello! I'm Ghost AI, Ridoan Zisan's virtual assistant. Ask me anything about him!"
    : "হ্যালো! আমি ঘোস্ট এআই, রিদোয়ান জিসানের ভার্চুয়াল সহকারী। তার সম্পর্কে কিছু জিজ্ঞাসা করুন!";
  ...
  // Type welcome message when chat opens
  useEffect(() => {
    if (isChatOpen && messages.length === 0 && !showWelcome) {
      setShowWelcome(true);
      typeWelcomeMessage();
    }
  }, [isChatOpen]);
  ...
  const typeWelcomeMessage = async () => {
    ...
    for (let i = 0; i < welcomeMessage.length; i++) {
      ...
      setWelcomeTypingText(welcomeMessage.substring(0, i + 1));
    }
  ...
  }, [messages, typingText, welcomeTypingText]);
  ...
  const handleKeywordClick = async (key: string) => {
    ...
    const keywordLabel = quickKeywords[lang].find(k => k.key === key)?.label || key;
            ...
            <div className="h-80 overflow-y-auto p-4 space-y-4 bg-secondary/30">
              ...
              {showWelcome && messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <div className="bg-card shadow-sm rounded-2xl p-3 max-w-[90%] mb-4">
                  ...
                  {welcomeComplete && (
                    ...
                    >
                      {quickKeywords[lang].map((keyword) => (
                        <button
                          key={keyword.key}
                          onClick={() => handleKeywordClick(keyword.key)}
                          className="px-3 py-1.5 bg-primary/10 text-primary text-xs font-medium rounded-full hover:bg-primary hover:text-primary-foreground transition-all"
                        >
                          {keyword.label}
                        </button>
                      ))}
                      ...
                      >
                        <p className="text-sm whitespace-pre-line" dangerouslySetInnerHTML={{ 
                          __html: message.content.replace(
                            /(https?:\/\/[^\s]+|[\w.-]+\.(com|org|app|io|net|dev)(\/[^\s]*)?)/g, 
                            (match) => {
                              const url = match.startsWith('http') ? match : `https://${match}`;
                              return `<a href="${url}" target="_blank" rel="noopener noreferrer" class="text-primary underline hover:opacity-80">${match}</a>`;
                            }
                          )
                        }} />
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      {quickKeywords[lang].map((keyword) => (
                        <button
                          key={keyword.key}
                          onClick={() => handleKeywordClick(keyword.key)}
                          className="px-2.5 py-1 bg-accent text-accent-foreground text-xs rounded-full hover:bg-primary hover:text-primary-foreground transition-all"
                        >
                          {keyword.label}
                        </button>
                      ))}
