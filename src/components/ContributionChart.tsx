...
const ContributionChart: React.FC = () => {
  ...
  const totalContributions = Object.values(contributionData).flat().reduce((sum: number, item: { month: string; contributions: number }) => sum + item.contributions, 0);
    ...
    <div className="bg-card rounded-xl p-4 md:p-6 shadow-sm border border-border mt-8">
      ...
      <div className="mt-4 pt-4 border-t border-border">
        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-12 gap-2">
          {(contributionData[2024] as Array<{ month: string; contributions: number }>).map((item, index) => (
